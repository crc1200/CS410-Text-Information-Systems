...
class TextRetrieval():
  ...
  def text2BitVector(self, text):
    ...
    bitVector = np.zeros(self.vocab.size)

    word_list = text.split(" ")

    for i in range(self.vocab.size):
      if self.vocab[i] in word_list:
        bitVector[i] = 1
  ...
  def execute_search_BitVec(self,query):
    ...
    for i, doc in enumerate(self.dataset[2]):
      score = self.bit_vector_score(query, doc)
      relevances[i] = score
...
if __name__ == '__main__':
    ...
    queries = ["movie where young girl finds a rabbit and goes to a magical world with a queen, fantasy, fun"]
    print("#########\n")
